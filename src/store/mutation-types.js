// 包含n个mutation的type名称常量
export const RECEIVE_ADDRESS = 'receive_address'//接收地址
export const RECEIVE_CATEGORYS = 'receive_categorys'//接收食品组分类
export const RECEIVE_SHOPS = 'receive_shops'//接收商家数组
export const RECEIVE_USER_INFO = 'receive_user_info'//接收用户信息
export const RESET_USER_INFO = 'reset_user_info'//重置用户信息
export const RECEIVE_INFO = 'receive_info'//接收商家信息
export const RECEIVE_GOODS = 'receive_goods'//接商品信息
export const RECEIVE_RATINGS = 'receive_ratings'//接收用户评价

export const INREMENT_FOOD_COUNT = 'increment_food_count'//增加food中count值
export const DEREMENT_FOOD_COUNT = 'decrement_food_count'//减少food中count值
export const CLEAR_CART = 'clear_cart'//清空购物车


